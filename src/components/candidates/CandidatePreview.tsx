...
export default function CandidatePreview({ candidate, onClose, onNext, onPrevious }: CandidatePreviewProps) {
  ...
  // Reset editing state when candidate changes
  useEffect(() => {
    setIsEditing(false);
  }, [candidate.id]);
  ...
  return (
    <div className={`bg-white/90 rounded-lg shadow-sm overflow-hidden ${isFullScreen ? 'fixed inset-0 z-50' : 'h-full'}`}>
                ...
                >
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm text-gray-500">{t('candidate.yearsOfExperience')}</p>
      ...
      <EditCandidateModal
        ...
        key={candidate.id} // Force re-mount of modal when candidate changes
        candidate={candidate} 
      />
    </div>
  );
