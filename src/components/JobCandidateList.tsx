...
export default function JobCandidateList({ jobId }: JobCandidateListProps) {
  ...
  const { documents: candidateJobs, add: addCandidateJob, update: updateCandidateJob, remove: removeCandidateJob } = useCandidateJobs(null, jobId);
  ...
  useEffect(() => {
    const count = candidateJobs.filter(cj => 
      cj.status === 'in_progress' || cj.status === 'matched'
    ).length;
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4">
        {candidateJobs.map((candidateJob) => {
          ...
          if (!candidate || candidateJob.status === 'inactive') return null;
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="font-medium text-gray-900">{candidate.name} {candidate.surname}</h3>
